...
for i = iStart:1:(iEnd-1)
    ...
    if (bit == 1)
        ...
        %Gets the prediction.
        pA = silhouetteFromCloud(dec.predictionPointCloud.Location, dec.pcLimit+1, dec.dimensionSliced, i, i, false);
        
        %Decodes the current image.
        [A, cabac] = decodeImageBAC_withMask_3DContexts_Inter(A, idx_i, idx_j, Yleft, pA, cabac);
...
if (bit == 1)    
    ...
    %Gets the prediction.
    pA = silhouetteFromCloud(dec.predictionPointCloud.Location, dec.pcLimit+1, dec.dimensionSliced, iEnd, iEnd, false);
